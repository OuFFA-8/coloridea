...
import { AutoplayVideoDirective } from '../client-cameras/autoplay-video.directive';
...
@Component({
  selector: 'app-project-screens',
  standalone: true,
  imports: [CommonModule, TranslateModule, AutoplayVideoDirective],
  templateUrl: './project-screens.html',
...
export class ProjectScreens implements OnInit, OnDestroy {
  ...
  private getSelectedProjectId(): string {
    try {
      ...
      return parsed?._id || parsed?.id || parsed?.projectId || '';
    } catch {
      return '';
    }
  }
  ...
  async ngOnInit() {
    ...
    const saved = localStorage.getItem('ci-project-screens-layout');
    if (saved) this.layoutId = saved;

    const savedSlots = localStorage.getItem('ci-project-screens-slots');
    if (savedSlots) {
  ...
  async loadCameras() {
    try {
      ...
      this.cameras = (res.data || []).filter(
        (c: Camera) =>
          c.isActive !== false &&
          c.type === 'project' &&
          (!this.currentProjectId || c.project === this.currentProjectId),
      );
  ...
  setLayout(id: string) {
    this.layoutId = id;
    localStorage.setItem('ci-project-screens-layout', id);
  ...
  assignCameraToSlot(cameraId: string) {
    ...
    localStorage.setItem('ci-project-screens-slots', JSON.stringify(obj));
